      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 py-24 space-y-16">
        ...
        <section className="space-y-6 max-w-3xl pt-10">
          ...
          <p className="text-lg text-[#ebebeb] max-w-xl leading-relaxed">
            ...
            and get a weekly reflection on what drains you.
          </p>
          
          <div className="flex flex-wrap gap-4 pt-4">
            {/* FIXED LINKS FOR DEMO FLOW */}
            <Link href="/dashboard#log" className="px-10 py-4 bg-[#072c3f] text-white rounded-full font-bold shadow-xl hover:bg-slate-800 transition-all active:scale-95">
              Try Demo
            </Link>
            <Link href="/dashboard#insights" className="px-10 py-4 bg-white/70 backdrop-blur-md text-slate-700 rounded-full font-bold border border-white/50 shadow-sm hover:bg-white transition-all active:scale-95">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 auto-rows-[250px]">
          ...
          <div className="md:col-span-2 md:row-span-2 p-10 bg-white/40 backdrop-blur-xl rounded-[3rem] border border-white/40 shadow-xl flex flex-col">
            <h3 className="text-2xl font-bold text-slate-900 mb-8 font-display">Top Priority Tasks</h3>
            
            <div className="space-y-4 flex-1 overflow-y-auto pr-2">
              {tasksPriority.slice(0, tasksPriority.length - 1).map(task => (
                <div key={task} className="flex items-center gap-4 p-5 bg-white/60 rounded-3xl border border-white/20 shadow-sm">
            ...
            {/* ALIGNMENT FIXED: Gemini Audit sub-box inside the card */}
          ...
          <div className="md:col-span-2 p-10 bg-white/40 backdrop-blur-xl rounded-[3rem] border border-white/40 shadow-xl flex flex-col">
            <h3 className="text-sm font-bold text-slate-900 uppercase tracking-[0.2em] mb-3">Stress Trend</h3>
          ...
          <div className="md:col-span-2 p-10 bg-[#072c3f] text-white rounded-[3rem] shadow-2xl flex flex-col justify-between">
            <div>
              <h3 className="text-indigo-300 text-sm font-bold uppercase tracking-[0.2em] mb-6">Mental Liquidity</h3>
              <p className="text-6xl font-bold mt-2">{stressLevel}<span className="text-2xl font-light opacity-50 ml-1">%</span></p>
              <p className="text-indigo-200/60 text-sm mt-1 font-medium">Capacity remaining this week</p>
            </div>
            <div className="w-full h-3 bg-white/10 rounded-full overflow-hidden">
              <div className="h-full bg-indigo-400 rounded-full transition-all duration-1000" style={{ width: `${stressLevel}%` }}></div>
            </div>
          </div>

          {/* Recommended Activities (Full Width) */}
          <div className="md:col-span-4 p-10 bg-white/40 backdrop-blur-xl rounded-[3rem] border border-white/40 shadow-xl">
            <div className="flex justify-between items-center mb-8">
              <h3 className="text-2xl font-bold text-slate-900 font-display italic">Recommended Assets</h3>
              <Link href="/relief" className="text-indigo-600 font-bold text-sm hover:underline">View Relief Center →</Link>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {['Indie Folk Playlist', 'Vagus Reset Walk'].map(task => (
                <div key={task} className="flex items-center gap-4 p-5 bg-white/50 rounded-2xl border border-white/20 hover:bg-white/80 transition-all cursor-pointer">
